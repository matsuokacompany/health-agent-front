...
export function LanguageSwitcher() {
  ...
  return <div className="language-switcher" ref={ref}><button className="button secondary icon-control" type="button" aria-label={t('language.change')} aria-haspopup="menu" aria-expanded={open} title={t('language.change')} onClick={() => setOpen((current) => !current)}><Globe aria-hidden="true" size={18} weight="duotone" /></button>{open ? <div className="language-menu" role="menu">{languageOptions.map((option) => <button className={option.locale === locale ? 'is-current' : ''} key={option.locale} type="button" role="menuitemradio" aria-checked={option.locale === locale} onClick={() => { setLocale(option.locale); setOpen(false); }}>{option.label}</button>)}</div> : null}</div>;
}
