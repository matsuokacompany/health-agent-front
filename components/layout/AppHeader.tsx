...
function PlatformSearch({ mobile = false }: { mobile?: boolean }) {
  ...
  return <form className={`platform-search ${mobile ? 'mobile-platform-search' : ''}`.trim()} role="search" onSubmit={submit}><label className="sr-only" htmlFor={id}>{t('header.searchLabel')}</label>{mobile ? <MagnifyingGlass className="platform-search-icon" aria-hidden="true" weight="bold" /> : null}<input id={id} name="search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={mobile ? t('header.mobileSearchPlaceholder') : t('header.searchPlaceholder')} /><button className="platform-search-submit" type="submit">Buscar</button></form>;
}
