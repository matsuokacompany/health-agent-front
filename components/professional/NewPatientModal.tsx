...
export function NewPatientModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const input = (name: keyof FormValues, label: string, props: React.InputHTMLAttributes<HTMLInputElement> & { mask?: InputMask } = {}) => {
    const { mask, ...inputProps } = props;
    return <label>{label}{errors[name] ? <span className="field-error" id={`${name}-error`}>{errors[name]}</span> : null}<input {...inputProps} name={name} value={values[name]} onChange={(event) => setValue(name, mask ? applyInputMask(mask, event.target.value) : event.target.value)} aria-invalid={Boolean(errors[name])} aria-describedby={errors[name] ? `${name}-error` : undefined} /></label>;
  };
  return <Modal open={open} title="Novo paciente" onClose={() => { if (!mutation.isPending && !isSavingAnamnese) onClose(); }}><form onSubmit={submit} noValidate>
    ...
    <div className="new-patient-grid">{input('name', 'Nome completo *', { autoComplete: 'name', maxLength: 120, required: true })}{input('email', 'E-mail *', { autoComplete: 'email', maxLength: 254, type: 'email', required: true })}{input('phone', 'Telefone', { autoComplete: 'tel', inputMode: 'tel', maxLength: 19, mask: 'phone', placeholder: '+55 (11) 99999-9999' })}{input('cpf', 'CPF', { inputMode: 'numeric', maxLength: 14, mask: 'cpf', placeholder: '000.000.000-00' })}{input('birth_date', 'Data de nascimento', { type: 'date' })}<label>Gênero<select name="gender" value={values.gender} onChange={(event) => setValue('gender', event.target.value)}><option value="">Não informado</option><option value="feminino">Feminino</option><option value="masculino">Masculino</option><option value="nao_binario">Não binário</option><option value="outro">Outro</option></select></label>{input('city', 'Cidade', { autoComplete: 'address-level2', maxLength: 100 })}{input('state', 'Estado', { autoComplete: 'address-level1', maxLength: 2, mask: 'state', placeholder: 'UF' })}</div>
