...
export function PatientAnamneseEditor({ patientId }: { patientId: string }) {
  ...
  async function save() {
    ...
    try {
      const updated = hasAnamnese
        ? await updatePatientAnamnese(patientId, payload)
        : await createPatientAnamnese(patientId, payload);
  ...
  return <section className="card professional-detail-section"><div className="professional-section-heading"><div><h2>Anamnese</h2><p className="muted compact">{hasAnamnese ? 'Registro clínico do paciente.' : 'Este paciente ainda não possui anamnese.'}</p></div>{record?.updated_at ? <small className="muted">Atualizada em {new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(record.updated_at))}</small> : null}</div>
    ...
    <div className="form-grid">
      <label>Alergias a medicamentos<textarea rows={3} maxLength={INPUT_LIMITS.allergies} value={medicationAllergies} onChange={(event) => setMedicationAllergies(event.target.value)} disabled={isLoadingAnamnese || isSavingAnamnese || Boolean(anamneseError)} placeholder="Ex.: Dipirona, Penicilina" /></label>
      <label>Alergias e restrições alimentares<textarea rows={3} maxLength={INPUT_LIMITS.allergies} value={foodRestrictions} onChange={(event) => setFoodRestrictions(event.target.value)} disabled={isLoadingAnamnese || isSavingAnamnese || Boolean(anamneseError)} placeholder="Ex.: Lactose, amendoim" /></label>
    </div>
