...
export type PatientDashboardCalendarDay = { date: string; status?: string; has_checkin: boolean; completed: boolean; pending: boolean; has_symptoms: boolean; had_symptoms?: boolean | null; checkins: DashboardCheckIn[] };
...
function normalizeOverview(value: unknown): PatientDashboardOverview { const source = isRecord(value) ? value : {}; const monitoring = isRecord(source.monitoring) ? source.monitoring : null; const activePlan = monitoring ? { id: String(monitoring.title ?? 'active-monitoring'), name: stringOrNull(monitoring.title) ?? 'Plano de acompanhamento', status: monitoring.active ? 'active' : 'inactive', active: Boolean(monitoring.active), starts_at: dateOnly(monitoring.start_date), ends_at: dateOnly(monitoring.end_date) } : (source.activePlan ?? source.active_plan ?? source.plan ?? null) as DashboardPlan | null; const statisticsSource = isRecord(source.statistics) && source.statistics.answered !== undefined ? [{ label: 'answered', value: numberFrom(source.statistics.answered) }] : (source.statistics ?? source.stats ?? source.summary); return { patient: (source.patient ?? source.user) as PatientDashboardOverview['patient'], activePlan, professionals: normalizeArray<DashboardProfessional>(source.professionals ?? source.team), anamnesisSummary: normalizeAnamnesisSummary(source.anamnesisSummary ?? source.anamnesis_summary ?? source.anamnese_summary), todayCheckIn: (source.todayCheckIn ?? source.today_checkin ?? source.today_check_in ?? null) as DashboardCheckIn | null, nextCheckIn: (source.nextCheckIn ?? source.next_checkin ?? source.next_check_in ?? null) as DashboardCheckIn | null, statistics: normalizeStatisticList(statisticsSource), alerts: normalizeArray<DashboardAlert>(source.alerts) }; }
