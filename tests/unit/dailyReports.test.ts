...
describe('dailyReportsApi', () => {
  ...
  it('gets, answers, and clears a calendar check-in by report id', async () => {
    ...
    await dailyReportsApi.update(42, { had_symptoms: true, symptom_description: 'Dor de cabeça' });
    await dailyReportsApi.removeResponse(42);

    expect(apiMock).toHaveBeenNthCalledWith(1, '/api/daily-reports/42');
    expect(apiMock).toHaveBeenNthCalledWith(2, '/api/daily-reports/42', { method: 'PATCH', body: JSON.stringify({ had_symptoms: true, symptom_description: 'Dor de cabeça' }) });
