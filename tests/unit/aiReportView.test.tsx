...
describe('detalhe clínico', () => {
  it('renderiza detalhe clínico sem JSON bruto e sem afirmar diagnóstico', () => { render(<AiReportResult report={report()} />); expect(screen.getByText('Resumo seguro')).toBeTruthy(); expect(screen.getByText('Possibilidade A')).toBeTruthy(); expect(screen.getByText(/não são diagnósticos confirmados/i)).toBeTruthy(); expect(screen.queryByText(/"avaliacao_clinica"/)).toBeNull(); });
  it('renderiza falha de geração', () => { render(<AiReportResult report={report({ status: 'FAILED', failure_code: 'MODEL_ERROR', ai: null })} />); expect(screen.getByText(/A geração falhou/)).toBeTruthy(); expect(screen.getByText('Falha na geração')).toBeTruthy(); });
  it('renderiza geração em andamento', () => { render(<AiReportResult report={report({ status: 'PROCESSING', ai: null })} />); expect(screen.getByText('Gerando interpretação')).toBeTruthy(); expect(screen.getByText(/continua sendo processada/)).toBeTruthy(); });
});
