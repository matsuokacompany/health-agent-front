...
import { ClinicalImagesSection, type ClinicalImagesSectionHandle } from '@/components/clinical-images/ClinicalImagesSection';

function isCompletedReport(report: DailyReport) {
  return Boolean(report.completed) || String(report.status ?? '').toUpperCase() === 'COMPLETED';
}

type MedicationAdherenceLevel = 'ALL' | 'PARTIAL' | 'NONE';
type EditFormState = {
  had_symptoms: boolean;
  symptom_description: string;
  diet_adherence: boolean;
  exercise_adherence: boolean;
  medication_adherence_level: MedicationAdherenceLevel;
...
function normalizeEditForm(report: DailyReport): EditFormState {
  return {
    ...
    diet_adherence: Boolean(report.diet_adherence),
    exercise_adherence: Boolean(report.exercise_adherence),
    medication_adherence_level: report.medication_adherence_level ?? (report.medication_adherence ? 'ALL' : 'NONE'),
    lifestyle_notes: String(report.lifestyle_notes ?? ''),
  };
}
...
export default function PatientMonitoring() {
  ...
  const [editForm, setEditForm] = useState<EditFormState>({
    ...
    exercise_adherence: false,
    medication_adherence_level: 'NONE',
    lifestyle_notes: '',
  });
  const imagesRef = useRef<ClinicalImagesSectionHandle>(null);
  ...
  async function submitEdit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = {
        ...(editForm.had_symptoms
          ? { had_symptoms: true, symptom_description: editForm.symptom_description }
          : { had_symptoms: false }),
        diet_adherence: editForm.diet_adherence,
        exercise_adherence: editForm.exercise_adherence,
        medication_adherence: editForm.medication_adherence_level === 'ALL',
        medication_adherence_level: editForm.medication_adherence_level,
        lifestyle_notes: editForm.diet_adherence ? '' : editForm.lifestyle_notes,
      };
      await dailyReportsApi.update(reportIdFromCheckIn(selectedCheckin), payload);
      try {
        await imagesRef.current?.submitPending();
      } catch {
        // The answer itself already saved -- surface the image failure on
        // its own instead of making the whole save look like it failed.
        setEditing(false);
        await loadDashboard();
        setFeedback('Resposta salva, mas a imagem não pôde ser enviada. Tente novamente pela galeria de imagens.');
        return;
      }
      setEditing(false);
      await loadDashboard();
  ...
  return <div className="patient-monitoring-page">
    {!hasMonitoring ? <EmptyState title={t('monitoring.notStarted')} /> : null}
    ...
    {loading ? <CalendarSkeleton /> : <section className="calendar-layout is-calendar-only"><Card className="calendar-card" data-tour="monitoring-calendar"><div className="calendar-header"><div><h2>{t('monitoring.monthlyCalendar')}</h2><p className="muted compact">{monthLabel}</p></div><div className="calendar-nav"><button className="button secondary" type="button" onClick={() => setVisibleMonth(firstOfMonth(new Date()))}>{t('monitoring.today')}</button><button className="button secondary icon-control" type="button" aria-label={t('monitoring.previousMonth')} disabled={!canGoPrevious} onClick={() => setVisibleMonth((current) => addMonths(current, -1))}>‹</button><button className="button secondary icon-control" type="button" aria-label={t('monitoring.nextMonth')} disabled={!canGoNext} onClick={() => setVisibleMonth((current) => addMonths(current, 1))}>›</button></div></div><div className="calendar-weekdays">{weekdays.map((day) => <span key={day}>{day}</span>)}</div><div className="calendar">{calendarCells.map((cell) => { if (cell.type === 'empty') return <span className="day calendar-empty-cell" aria-hidden="true" key={cell.key} />; const day = cell.day; const status = getDayStatus(day); const adherenceLabel = [day.diet_followed ? t('monitoring.diet') : null, day.exercise_followed ? t('monitoring.exercise') : null, day.medication_taken ? t('monitoring.medication') : null, day.medication_partial ? t('monitoring.legendMedicationPartial') : null].filter(Boolean).join(', '); return <button className={`day ${getDayClassName(day)} ${selected?.date === day.date ? 'is-active' : ''} ${isToday(day.date) ? 'is-today' : ''}`} key={day.date} type="button" aria-label={`${formatDateForLocale(day.date, locale)}: ${statusLabel(status)}${adherenceLabel ? ` — ${adherenceLabel}` : ''}`} onClick={() => openDayDetails(day)}><strong>{new Date(`${day.date}T00:00:00`).getDate()}</strong><span>{statusLabel(status)}</span>{day.diet_followed || day.exercise_followed || day.medication_taken || day.medication_partial ? <span className="day-adherence-icons" aria-hidden="true">{day.diet_followed ? <ForkKnife weight="duotone" className="icon-diet" /> : null}{day.exercise_followed ? <PersonSimpleRun weight="duotone" className="icon-exercise" /> : null}{day.medication_taken ? <Pill weight="duotone" className="icon-medication" /> : null}{day.medication_partial ? <CircleHalf weight="duotone" className="icon-medication-partial" /> : null}</span> : null}</button>; })}</div><div className="calendar-legend" aria-label={t('monitoring.legend')} data-tour="monitoring-legend"><div className="calendar-legend-group"><span className="calendar-legend-group-title">{t('monitoring.legendStatusTitle')}</span><span><i className="legend-complete" />{t('monitoring.legendAnsweredNoSymptoms')}</span><span><i className="legend-symptom" />{t('monitoring.legendAnsweredSymptoms')}</span><span><i className="legend-pending" />{t('monitoring.legendUnanswered')}</span><span><i className="legend-issue" />{t('monitoring.legendIncomplete')}</span><span><i className="legend-empty" />{t('monitoring.legendNoCheckIn')}</span></div><div className="calendar-legend-group calendar-legend-icons"><span className="calendar-legend-group-title">{t('monitoring.legendAnswersTitle')}</span><span><ForkKnife aria-hidden="true" weight="duotone" className="icon-diet" /> {t('monitoring.legendDiet')}</span><span><PersonSimpleRun aria-hidden="true" weight="duotone" className="icon-exercise" /> {t('monitoring.legendExercise')}</span><span><Pill aria-hidden="true" weight="duotone" className="icon-medication" /> {t('monitoring.legendMedication')}</span><span><CircleHalf aria-hidden="true" weight="duotone" className="icon-medication-partial" /> {t('monitoring.legendMedicationPartial')}</span></div></div></Card></section>}

    <Modal open={detailsOpen} title={selected ? formatDateForLocale(selected.date, locale) : t('monitoring.dayActions')} onClose={() => setDetailsOpen(false)}>{selected ? <div className="stack">{selected.checkins.length ? selected.checkins.map((checkin) => { const report = selectedCheckin?.id === checkin.id && selectedReport ? selectedReport : checkin; const completed = Boolean(report.completed) || String(report.status ?? '').toUpperCase() === 'COMPLETED'; return <div className="checkin-detail" key={checkin.id}>{saving && selectedCheckin?.id === checkin.id && !selectedReport ? <p className="muted">{t('monitoring.loadingDetails')}</p> : <><p><strong>{t('monitoring.symptoms')}:</strong> {report.had_symptoms != null ? (report.had_symptoms ? t('monitoring.yes') : t('monitoring.no')) : t('monitoring.awaitingAnswer')}</p>{report.symptom_description ? <p><strong>{t('monitoring.descriptionLabel')}:</strong> {report.symptom_description}</p> : completed ? <p className="muted">{t('monitoring.noDescription')}</p> : null}{(report.suspected_cause ?? report.cause) ? <p><strong>{t('monitoring.cause')}:</strong> {report.suspected_cause ?? report.cause}</p> : null}{report.diet_adherence != null ? <p><strong>{t('monitoring.diet')}:</strong> {report.diet_adherence ? t('monitoring.yes') : t('monitoring.no')}</p> : null}{report.diet_adherence === false ? <p><strong>{t('monitoring.dietDeviationLabel')}:</strong> {report.lifestyle_notes || t('monitoring.noDietDeviation')}</p> : null}{report.exercise_adherence != null ? <p><strong>{t('monitoring.exercise')}:</strong> {report.exercise_adherence ? t('monitoring.yes') : t('monitoring.no')}</p> : null}{(report.medication_adherence != null || report.medication_adherence_level) ? <p><strong>{t('monitoring.medication')}:</strong> {report.medication_adherence_level === 'PARTIAL' ? t('monitoring.medicationPartialOption') : (report.medication_adherence ? t('monitoring.yes') : t('monitoring.no'))}</p> : null}{!completed ? <p className="muted">{t('monitoring.checkInInProgress')}</p> : null}</>}<div className="page-actions"><button className="button secondary" disabled={saving} type="button" onClick={() => openEditModal(checkin)}>{completed ? t('monitoring.editAnswer') : t('monitoring.answerCheckIn')}</button>{completed ? <button className="button danger-button" disabled={saving} type="button" onClick={() => { setSelectedCheckin(checkin); setDeleting(true); }}>{t('monitoring.deleteAnswer')}</button> : null}</div></div>; }) : <p className="muted">{t('monitoring.noReportForDay', { status: statusLabel(getDayStatus(selected)) })}</p>}</div> : null}</Modal>
    <Modal open={editing} title={t('monitoring.editTitle')} onClose={() => setEditing(false)}>{selectedCheckin ? <div className="stack checkin-edit-content"><form id="checkin-edit-form" className="stack" onSubmit={submitEdit}><label>{t('monitoring.symptoms')}<select name="had_symptoms" value={editForm.had_symptoms ? 'yes' : 'no'} onChange={(event) => setEditForm((current) => ({ ...current, had_symptoms: event.target.value === 'yes' }))}><option value="no">{t('monitoring.no')}</option><option value="yes">{t('monitoring.yes')}</option></select></label>{editForm.had_symptoms ? <label>{t('monitoring.descriptionLabel')}<textarea name="symptom_description" rows={4} value={editForm.symptom_description} onChange={(event) => setEditForm((current) => ({ ...current, symptom_description: event.target.value }))} /></label> : null}<label>{t('monitoring.diet')}<select name="diet_adherence" value={editForm.diet_adherence ? 'yes' : 'no'} onChange={(event) => setEditForm((current) => ({ ...current, diet_adherence: event.target.value === 'yes' }))}><option value="no">{t('monitoring.no')}</option><option value="yes">{t('monitoring.yes')}</option></select><small className="muted">{t('monitoring.dietHelp')}</small></label>{!editForm.diet_adherence ? <label>{t('monitoring.dietDeviationLabel')}<textarea name="lifestyle_notes" rows={3} value={editForm.lifestyle_notes} onChange={(event) => setEditForm((current) => ({ ...current, lifestyle_notes: event.target.value }))} /></label> : null}<label>{t('monitoring.exercise')}<select name="exercise_adherence" value={editForm.exercise_adherence ? 'yes' : 'no'} onChange={(event) => setEditForm((current) => ({ ...current, exercise_adherence: event.target.value === 'yes' }))}><option value="no">{t('monitoring.no')}</option><option value="yes">{t('monitoring.yes')}</option></select><small className="muted">{t('monitoring.exerciseHelp')}</small></label><label>{t('monitoring.medication')}<select name="medication_adherence_level" value={editForm.medication_adherence_level} onChange={(event) => setEditForm((current) => ({ ...current, medication_adherence_level: event.target.value as MedicationAdherenceLevel }))}><option value="NONE">{t('monitoring.no')}</option><option value="PARTIAL">{t('monitoring.medicationPartialOption')}</option><option value="ALL">{t('monitoring.yes')}</option></select><small className="muted">{t('monitoring.medicationHelp')}</small></label></form><ClinicalImagesSection ref={imagesRef} hideSubmitButton patientId={patientId} dailyReportId={Number(reportIdFromCheckIn(selectedCheckin))} title="Imagens deste check-in" /><div className="page-actions checkin-edit-actions"><button className="button secondary" type="button" onClick={() => setEditing(false)}>{t('monitoring.cancel')}</button><button className="button" form="checkin-edit-form" disabled={saving} type="submit">{saving ? t('monitoring.saving') : t('monitoring.saveAnswer')}</button></div></div> : null}</Modal>
