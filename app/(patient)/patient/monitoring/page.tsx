...
export default function PatientMonitoring() {
  ...
  const patientId = [user?.patient_id, overview?.patient?.id, selectedReport?.patient_id]
    .map(Number).find((id) => Number.isSafeInteger(id) && id > 0);
