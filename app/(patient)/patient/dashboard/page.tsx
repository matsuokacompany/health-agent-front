...
export default function PatientDashboard() {
  ...
  return <><PageHeader eyebrow="Portal do paciente" title={`Olá, ${user?.name ?? 'paciente'}`} description="Acompanhe seus check-ins e a evolução do monitoramento." action={<Button href="/patient/monitoring">Abrir monitoramento</Button>} />
    ...
    <Card><h2>Últimas mensagens do período</h2>{filteredReports.length ? filteredReports.slice(0, 8).map((report) => <div className="list-row" key={report.id}><span>{report.report_date ?? 'Registro sem data'}</span><StatusBadge status={report.status} /></div>) : <EmptyState description="Nenhuma mensagem encontrada para o período selecionado." />}</Card></>}
  </>;
}
