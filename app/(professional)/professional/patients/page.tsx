...
export default function Patients() {
  ...
  return <>
    ...
    <section className="grid" data-tour="patients-metrics"><MetricCard icon={<UsersThree aria-hidden="true" size={22} weight="bold" />} label="Pacientes ativos" value={activeCount} tone="info" /><MetricCard icon={<Stethoscope aria-hidden="true" size={22} weight="bold" />} label="Relatos de sintomas" value={symptomCount} tone={symptomCount > 0 ? 'warn' : 'ok'} /><article className="card"><label>Buscar paciente<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nome ou e-mail" /></label></article></section>
