...
function protectedPatientLabel(patientId: string) { return `Paciente #${patientId}`; }
...
function AiReportDetails({ report, showSensitive }: { report: ProfessionalAiReportResponse; showSensitive: boolean }) {
  if (!showSensitive) return <p className="notice">Relatório gerado. Ative “Exibir informações sensíveis” para visualizar o conteúdo clínico.</p>;
...
export default function PatientDetail({ params }: { params: Promise<{ patientId: string }> }) {
  ...
  const displayName = showSensitive ? data?.user?.name : protectedPatientLabel(patientId);
  ...
  return <div className="professional-patient-detail">
    ...
    <div className="topbar professional-detail-topbar"><span className="badge">Prontuário vinculado</span><div className="professional-detail-actions"><button className="button secondary" type="button" onClick={() => setShowSensitive((current) => !current)}>{showSensitive ? 'Ocultar informações sensíveis' : 'Exibir informações sensíveis'}</button><Link href="/professional/patients">← Pacientes</Link></div></div>
    <header className="page-header professional-detail-header"><div><span className="eyebrow">Visão 360°</span><h1>{displayName ?? 'Prontuário do paciente'}</h1><p className="muted">Dashboard profissional com check-ins, anamnese e relatório clínico por IA. Dados clínicos ficam ocultos por padrão nesta tela.</p></div></header>
