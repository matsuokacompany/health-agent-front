...
export default function Page(){const id=Number(useParams().id);const[info,setInfo]=useState('');const[loading,setLoading]=useState(true);useEffect(()=>{setLoading(true);anamnesesApi.byUser(id).then(a=>setInfo(safeClinicalText(a))).catch(()=>setInfo('')).finally(()=>setLoading(false))},[id]);return <><h1>Anamnese do paciente</h1>{loading?<article className="card" aria-busy="true"><SkeletonBlock/><SkeletonBlock/><SkeletonBlock/></article>:info?<article className="card"><p className="readonly-clinical-text">{info}</p></article>:<EmptyState title="Anamnese não encontrada"/>}</>}
