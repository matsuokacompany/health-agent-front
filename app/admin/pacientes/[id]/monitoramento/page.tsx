...
export default function Page() {
  ...
  return <><h1>Monitoramento</h1><form className="card stack" onSubmit={submit} noValidate>
    ...
    <div className="new-patient-grid"><label>Período de acompanhamento<DateField value={form.start_date} onChange={(iso) => setValue('start_date', iso)} /></label><label>Data final<DateField min={form.start_date || undefined} value={form.end_date} onChange={(iso) => setValue('end_date', iso)} ariaInvalid={Boolean(errors.end_date)} ariaDescribedBy={errors.end_date ? 'end_date-error' : undefined} />{errors.end_date ? <span className="field-error" id="end_date-error" role="alert">{errors.end_date}</span> : null}</label></div>
